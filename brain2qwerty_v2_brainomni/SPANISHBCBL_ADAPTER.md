...
python -m brain2qwerty_v2_brainomni.main cache --debug
python -m brain2qwerty_v2_brainomni.main debug
python -m brain2qwerty_v2_brainomni.main train
python -m brain2qwerty_v2_brainomni.main eval --ckpt /path/to/checkpoint.ckpt
...
- MEG 数值预处理：仅选 MEG 通道，重采样至 256 Hz，0.5–85 Hz 带通，
  `[50, 60]` 陷波配置；不使用 `RobustScaler` 或 clamp，而是在句子样本级
  进行逐通道 z-score，不做 baseline correction。
